...
def monitor():
    ...
    with open(log, 'w') as log_file:
        log_file.write('')
    rospy.init_node('safety_distance', anonymous=True)
    pub_error = rospy.Publisher(name = 'safety_distance/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
    pub_verdict = rospy.Publisher(name = 'safety_distance/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
    rospy.Subscriber('/front/scan', LaserScan, callback_front_scan)
...
def main(argv):
    ...
    log = '/catkin_ws/src/monitor/log_safety_distance.txt'
