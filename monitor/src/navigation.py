...
def callbackamcl_pose(data):
    ...
    ws_lock.release()
    rospy.loginfo('event propagated to oracle')
def callback_move_base_result(data):
    global ws, ws_lock
    rospy.loginfo('monitor has observed: ' + str(data))
    dict = message_converter.convert_ros_message_to_dictionary(data)
    dict['topic'] = '/move_base/result'
    dict['time'] = rospy.get_time()
    ws_lock.acquire()
    while dict['time'] in dict_msgs:
        dict['time'] += 0.01
...
def monitor():
    ...
    pub_error = rospy.Publisher(name = 'navigation/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
    pub_verdict = rospy.Publisher(name = 'navigation/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
    rospy.Subscriber('command', Command, callbackcommand)
    rospy.Subscriber('amcl_pose', PoseWithCovarianceStamped, callbackamcl_pose)
    rospy.Subscriber('/move_base/result', MoveBaseActionResult, callback_move_base_result)
