...
def monitor():
    ...
    with open(log, 'w') as log_file:
        log_file.write('')
    rospy.init_node('radiation_agent', anonymous=True)
    pub_error = rospy.Publisher(name = 'radiation_agent/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
    pub_verdict = rospy.Publisher(name = 'radiation_agent/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
